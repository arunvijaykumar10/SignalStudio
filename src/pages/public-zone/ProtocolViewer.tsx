...
// Define TypeScript interfaces for the protocol objects
interface CTAObject {
  text: string;
  url: string;
}
...
interface ProtocolObjects {
  [key: string]: ProtocolObject;
}
...
const SignalProtocolViewer = () => {
  ...
  const protocolObjects: ProtocolObjects = {
    'v1.0': {
  ...
  const getDiff = (key: string | null, path: string[] = []): DiffInfo => {
  ...
  const renderJSONTree = (obj: any, path: string[] = [], level = 0): React.ReactNode => {
    ...
    return (
      <div className="ml-4">
        {keys.map((key) => {
          ...
          const diffInfo = compareMode ? getDiff(key.toString(), newPath) : { changed: false, added: false, removed: false };
  ...
  const currentObject: Partial<ProtocolObject> = protocolObjects[selectedVersion] || {};
      ...
      <div className="flex mb-4 gap-4">
        <div className="bg-gray-50 p-4 rounded-md flex-shrink-0 w-1/4">
          ...
          <div className="text-sm space-y-2">
            ...
            <div><span className="font-medium">Created:</span> {currentObject.created_at ? new Date(currentObject.created_at).toLocaleString() : ''}</div>
            <div>
        ...
        <div className="flex-1 border rounded-md overflow-hidden">
          ...
          <div className="p-4 overflow-auto font-mono text-sm">
            ...
            {expandedSections.metadata && currentObject.metadata && renderJSONTree(currentObject.metadata, ['metadata'])}
            ...
            {expandedSections.content && currentObject.content && renderJSONTree(currentObject.content, ['content'])}
            ...
            {expandedSections.tags && currentObject.tags && renderJSONTree(currentObject.tags, ['tags'])}
            ...
            {expandedSections.version && currentObject.version_info && renderJSONTree(currentObject.version_info, ['version_info'])}
          </div>
        </div>
      </div>
...
export default SignalProtocolViewer;