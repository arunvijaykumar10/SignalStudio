...
const ConnectedAccountsTab = () => {
  ...
  const [connections, setConnections] = useState<{
    ...
    slack: { connected: true, username: '@jonathan', lastSync: 'April 16, 9:42 AM' },
  ...
  const handleConnectionToggle = (service: string) => {
    if (service === 'slack' && connections.slack.connected) {
      // Disconnect flow
      if (confirm('Are you sure you want to disconnect Slack? This will affect notifications and exports.')) {
        setConnections({
          ...connections,
          slack: {
              connected: false,
      ...
      if (service === 'slack') {
        setConnections({
          ...connections,
          slack: { connected: true, username: '@jonathan', lastSync: 'Just now' }
        });
  ...
  return (
    <div className="w-full max-w-4xl mx-auto p-6 bg-white rounded-lg shadow-sm">
      <h2 className="text-2xl font-semibold text-gray-800 mb-6">Connected Accounts & Integrations</h2>
          ...
          <div className="flex items-center">
            <div className="bg-slate-100 p-2 rounded-lg mr-4">
              <svg className="w-8 h-8" viewBox="0 0 24 24" fill="none">
                <path d="M19.8752 11.2498C20.9106 11.2498 21.75 10.4103 21.75 9.37496C21.75 8.33959 20.9105 7.50017 19.8752 7.50017C18.8398 7.50017 18.0004 8.33959 18.0004 9.37496L18 14.625C18 15.6603 18.8395 16.4998 19.8748 16.4998C20.9102 16.4998 21.7496 15.6603 21.7496 14.625C21.7496 13.5897 20.9101 12.7502 19.8748 12.7502H19.8752Z" fill="#E01E5A"/>
                <path d="M4.12484 12.7498C3.08947 12.7498 2.25 13.5893 2.25 14.6246C2.25 15.66 3.08947 16.4994 4.12484 16.4994C5.16021 16.4994 5.99963 15.6599 5.99963 14.6246L6 9.37455C6 8.33918 5.16053 7.49976 4.12516 7.49976C3.08979 7.49976 2.25037 8.33918 2.25037 9.37455C2.25037 10.4099 3.08984 11.2494 4.12521 11.2494H4.12484V12.7498Z" fill="#36C5F0"/>
                <path d="M12.7498 4.12484C12.7498 3.08947 11.9103 2.25 10.875 2.25C9.83959 2.25 9.00017 3.08947 9.00017 4.12484C9.00017 5.16021 9.83959 5.99963 10.875 5.99963H16.1246C17.1599 5.99963 17.9994 5.16016 17.9994 4.12479C17.9994 3.08942 17.1599 2.25 16.1246 2.25C15.0892 2.25 14.2498 3.08947 14.2498 4.12484V4.12484H12.7498Z" fill="#2EB67D"/>
                <path d="M11.2498 19.8752C11.2498 20.9105 12.0893 21.75 13.1246 21.75C14.16 21.75 14.9994 20.9105 14.9994 19.8752C14.9994 18.8398 14.1599 18.0004 13.1246 18.0004H7.87496C6.83959 18.0004 6.00017 18.8399 6.00017 19.8752C6.00017 20.9106 6.83959 21.75 7.87496 21.75C8.91033 21.75 9.74976 20.9105 9.74976 19.8752V19.8752H11.2498Z" fill="#ECB22E"/>
              </svg>
            </div>
            <div>
        ...
        <div className="flex justify-between items-center">
          <div className="flex items-center">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500">Auto-send exports to my inbox</p>
            </div>
          </div>
          ...
          <div className="flex items-center">
            <button 
              ...
              className={`relative inline-flex h-6 w-11 items-center rounded-full ${connections.email.enabled ? 'bg-blue-600' : 'bg-gray-200'}`}
            >
              <span className="sr-only">Toggle email sync</span>
              <span 
                className={`inline-block h-4 w-4 transform rounded-full bg-white transition ${connections.email.enabled ? 'translate-x-6' : 'translate-x-1'}`} 
              />
            </button>
          </div>
        </div>
      ...
      <div className="mb-8 border rounded-lg p-5">
        <div className="mb-3">
          ...
          <p className="text-sm text-gray-500">For asset export or attachment upload</p>
        </div>
          ...
          <div className="p-4 border rounded-md">
            <div className="flex justify-between items-center">
              <div className="flex items-center">
                <div className="bg-slate-100 p-1 rounded-md mr-3">
                  <svg className="w-6 h-6" viewBox="0 0 24 24">
                    <path d="M6 6.15L12 1.5L18 6.15L12 10.8L6 6.15ZM18 12.15L12 16.8L6 12.15L12 7.5L18 12.15ZM6 18.15L12 13.5L18 18.15L12 22.8L6 18.15Z" fill="#0061FF"/>
                  </svg>
                </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-md w-full">
            <div className="text-center">
              <h3 className="text-lg font-medium mb-2">Testing {testTarget} Connection</h3>
              ...
              {!testResult ? (
                ...
                <div className={`flex flex-col items-center py-4 ${testResult.success ? 'text-green-600' : 'text-red-600'}`}>
                  {testResult.success ? (
